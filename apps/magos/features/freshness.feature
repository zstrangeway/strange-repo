Feature: Data that is never stale
  ...
  # magos can check often and download only when something actually moved.
  ...
  Scenario: Asked something before the first sync ever ran
    ...
    Then magos should say it has no data yet
    And magos should say how to sync
  ...
  Scenario: Checking costs one small request when nothing changed
    Given magos synced at the export's current timestamp
    When magos checks for updates
    Then magos should have fetched only the update marker
    And magos should not have downloaded any table

  Scenario: The export moved, so magos re-syncs
    Given magos synced at an older timestamp
    And the export has since been updated
    When magos checks for updates
    Then magos should download the tables again
  ...
  Scenario: A sync that dies halfway leaves the old data intact
    ...
    Then magos should still answer from the older complete data
    And the recorded timestamp should still be the older one
    And magos should report that the sync failed

  Scenario: A table that arrives malformed is not written
    Given magos has a complete sync from an older timestamp
    And the export has since been updated
    And one table comes back with columns magos does not recognise
    When magos syncs
    Then magos should still answer from the older complete data
    And magos should name the table it could not read
  ...
  Scenario: Offline, with data already synced
    ...
    Then magos should answer from what it has
  ...
  Scenario: Offline, having never synced
    ...
    When I look up a datasheet
    Then magos should say it has no data yet
    And magos should say it could not reach the export
  ...
  Scenario: Data old enough to be worth mentioning says so
    Given magos has a complete sync from well over a week ago
    And the export cannot be reached
    When I look up a datasheet
